...
def parseInput (input : String) :=
  input.toSubstring.splitOn "\n\n" |>.map fun machine =>
    match machine.splitOn "\n" with
    | [a, b, p] => {
      a := parseButton a,
      b := parseButton b,
      p :=
        let s := p.drop 9
        match s.splitOn ", Y=" with
        | [x, y] => (x.toNat?.get!, y.toNat?.get!)
        | _ => panic!"illegal input"
      : Machine
    }
    | _ => panic!"illegal input"
where parseButton (line : Substring) :=
  let s := line.drop 12
  let x := s.take 2 |>.toNat?.get!
  let y := s.takeRight 2 |>.toNat?.get!
  (x, y)

def toPart2 (machine : Machine) :=
  { machine with p := machine.p + 10000000000000n }

-- solve: (xa, ya) * a + (xb, yb) * b = (xp, yp)
-- a' := xa * a
-- b' := xb * b
-- a' + b' = xp
-- ya * a + yb * b = yp
-- ya * xb * a' + yb * xa * b' = yp * xa * xb
-- ya * xb * a' + yb * xa * (xp - a') = yp * xa * xb
-- (ya * xb - yb * xa) * a' + yb * xa * xp = yp * xa * xb
-- (ya * xb - yb * xa) * a = yp * xb - yb * xp
def solveMachine (machine : Machine) :=
  let (xa, ya) := machine.a
  let (xb, yb) := machine.b
  let (xp, yp) := machine.p
  let top := Int.ofNat yp * xb - yb * xp
  let bottom := Int.ofNat ya * xb - yb * xa
...
#eval input.map solveMachine |>.sum
#eval input.map (solveMachine ∘ toPart2) |>.sum
